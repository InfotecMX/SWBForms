...
/*
  ...
  Version v9.1p_2014-03-08/LGPL Deployment (2014-03-08)
...
if (window.isc && isc.version != "v9.1p_2014-03-08/LGPL Deployment") {
    isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
        + "SmartClient version '" + isc.version + "' and additional modules from 'v9.1p_2014-03-08/LGPL Deployment'. Mixing resources from different "
...
// +link{SplitPane.detailButtons} for details.
// <p>
...
isc.SplitPane.addProperties({
            ...
            (this.currentUIConfig === "landscape" && this.currentPane !== "navigation" && this._hasListPane()))
        {
            ...
            if (this.showNavigationPaneButton == null) {
                this.showNavigationPaneButton = this.createAutoChild(
                    "showNavigationPaneButton",
                    {
                        ...
                        click : function () {
                            if (this.creator.currentPane === "detail" &&
                                this.creator._hasListPane() &&
                                this.creator.currentUIConfig !== "landscape")
                            {
                    ...
                    isc.NavigationButton
                );
            } else {
                this.showNavigationPaneButton.setTitle(backButtonTitle);
            }

            var controls = [];
            if (this.currentUIConfig !== "portrait" || this._hasListPane()) {
                controls.add(this.showNavigationPaneButton);
            }
    ...
    // +link{SplitPane.navigationBar,navigationBar}. Note that the default behavior is to
    // automatically create and show a "back button" as the left button that allows
    // transitioning back to the +link{SplitPane.navigationPane,navigationPane} (tablet and
    // handset mode) or the +link{SplitPane.listPane,listPane} (handset mode). If
    // <code>showLeftButton</code> is true, then a left button is shown <em>in addition</em>
    ...
    // Setter for +link{NavigationBar.leftButtonTitle,leftButtonTitle} of the
    // +link{SplitPane.navigationBar,navigationBar}.
    //
    ...
    // +link{SplitPane.navigationBar,navigationBar}.
    // @param newTitle (HTMLString) new title for right button.
    // @visibility external
    //<
    setRightButtonTitle : function (newTitle) {
    ...
    // <p>
    // If, based on the +link{SplitPane.deviceMode,deviceMode} and +link{SplitPane.pageOrientation,pageOrientation}, this causes the
    // +link{SplitPane.navigationPane,navigationPane} to be hidden, the +link{SplitPane.leftButton,back button} will be updated
    // with the current title of the <code>navigationPane</code>, or the
    ...
    // will be used for the back button title if the user goes back to the <code>listPane</code>.
    //
    // @param [listPaneTitle] (HTMLString) optional new list title.
    // @param [backButtonTitle] (HTMLString) optional new title for the +link{SplitPane.leftButton,back button}.
    // @visibility external
    //<
    ...
    // to be hidden, the +link{SplitPane.leftButton,back button} will be updated
